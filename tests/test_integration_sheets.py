...
def _collect_runnable_test_cases(rows: list[dict[str, str]]) -> list[tuple[str, str, str]]:
    cases: list[tuple[str, str, str]] = []
    for row in rows:
        case_id = _first_value(row, "test_id", "case_id", "test_case_id")
        status = _first_value(row, "status", "test_status")
        message = _first_value(row, "message", "input_message", "user_message", "問い合わせ文", "メッセージ")
...
def _load_test_cases_or_skip() -> list[tuple[str, str, str]]:
    try:
...
def test_sheet_loading_smoke() -> None:
    ...
    assert aliases, "aliases シートが空です。"
...
def test_sheet_has_required_templates() -> None:
    ...
    template_ids = {
        _first_value(row, "template_id", "テンプレートID", "テンプレートid")
...
def test_generate_reply_against_test_cases() -> None:
    ...
    if not cases:
        pytest.skip("実行可能な test_cases 行が見つかりませんでした。status=pending の行のみです。")
    ...
    # 実シート照合ではシート再読込を避け、API の Read requests 制限に当たりにくくする。
    ...
    for case_id, message, expected_team_id, expected_team_name, expected_template_id, expected_items_count in cases:
        ...
        actual_template_id = "TMP-002" if not team or team.get("ステータス") != "有効" else choose_template_id(valid_items, team_items)

        assert actual_template_id == expected_template_id, (
            f"{case_id}: template mismatch. expected={expected_template_id}, actual={actual_template_id}, reply={reply}"
        )
